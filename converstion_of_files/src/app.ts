...
type ConversionType = 'pdf-to-excel' | 'pdf-to-word' | 'word-to-pdf' | 'excel-to-pdf' | 'auto';
...
class ConversionApp {
  ...
  private currentMode: ConversionType = 'pdf-to-excel';
  private history: ConversionHistoryItem[] = [];

  // DOM Elements
  private dropZone!: HTMLElement;
  private fileInput!: HTMLInputElement;
  private fileDetailsCard!: HTMLElement;
  ...
  private modeCards!: NodeListOf<HTMLElement>;
  ...
  private initElements(): void {
    ...
    this.fileDetailsCard = document.getElementById('file-details') as HTMLElement;
    this.fileNameEl = document.getElementById('file-name') as HTMLElement;
    this.fileSizeEl = document.getElementById('file-size') as HTMLElement;
    this.fileIconEl = document.getElementById('file-icon') as HTMLElement;
    this.removeFileBtn = document.getElementById('remove-file-btn') as HTMLButtonElement;
    ...
    this.modeCards = document.querySelectorAll('.mode-card') as NodeListOf<HTMLElement>;
  }
  ...
  private bindEvents(): void {
    ...
    this.modeCards.forEach(card => {
      card.addEventListener('click', () => {
        const mode = card.dataset.mode as ConversionType;
        if (mode) {
          this.setMode(mode);
        }
      });
  ...
  private setMode(mode: ConversionType): void {
  ...
  private updateModeUI(selectedMode: ConversionType): void {
    this.modeCards.forEach(card => {
  ...
  private validateFileForMode(file: File): boolean {
    const ext = file.name.split('.').pop()?.toLowerCase();
  ...
  private async startConversion(): Promise<void> {
    ...
    if (!this.validateFileForMode(this.selectedFile)) return;
  ...
  private addHistoryItem(item: ConversionHistoryItem): void {
  ...
  private getModeDisplayName(mode: ConversionType): string {
    switch (mode) {
  ...
  private showStatus(msg: string, type: 'success' | 'error'): void {
    this.statusMessage.className = `p-4 rounded-xl text-sm mt-4 ${
      type === 'success' ? 'bg-emerald-950/70 border border-emerald-500/50 text-emerald-200' : 'bg-rose-950/70 border border-rose-500/50 text-rose-200'
    }`;
